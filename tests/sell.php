<?php
  ...
  $sellOBJ  = $exchange->sell(array("_market" => $_market , "_currency" => $_currency , "rate" => $price , "amount" => $amount));
  debug($sellOBJ);

  echo "<h1>Method: getOrder()</h1>";
  if($sellOBJ["success"] == true) {
    $orderOBJ = $exchange->getOrder(array("orderid" => $sellOBJ["result"]["orderid"]));
    debug($orderOBJ);
  }  
?>
